...
def create_study_session(new_study_session: StudySessionCreate, session: Session = Depends(get_session)):
...
def end_study_session(study_session_id: int, session_end_data: StudySessionEnd, session: Session = Depends(get_session)):
    return crud_end_study_session(session, study_session_id, session_end_data)
